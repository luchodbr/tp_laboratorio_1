 ...
 * \param mensaje Es el mensaje a ser mostrado
 * \param input Array donde se cargará el texto ingresado
 * \return int 1 si el texto es alfa numerico
 */
 ...
 * \param input Array donde se cargará el texto ingresado
 * \param maxLenght Longitud maxima del texto ingresado
 * \param attemps Indica la cantidad de reintentos ante un error
...
int getValidDescripcion(char* requestMessage,char* errorMessageLenght,char* input,int maxLenght,int attemps)
{
    ...
    for(i=0;i<attemps;i++)
    {
        getStringDescripcion(requestMessage,buffer);
        if(strlen(buffer) >= maxLenght)
        {
            printf ("%s",errorMessageLenght);
            continue;
...
int getValidLink(char* requestMessage,char* errorMessage,char* errorMessageLenght,char* input,int maxLenght,int attemps)
{
    ...
    for(i=0;i<attemps;i++)
    {
        if (!getStringLink(requestMessage,buffer))
        {
            printf ("%s",errorMessage);
            continue;
        }
 ...
 * \param maxLenght Longitud maxima del texto ingresado
 * \param attemps Indica la cantidad de reintentos ante un error
 * \return 0 si consiguio el String Alfa numerico -1 si no
 */
 ...
 * \brief Verifica si el valor recibido contiene solo números, + y -
 * \param str Array con la cadena a ser analizada
 * \return 1 si contiene solo numeros y dos guiones.
 */
int esCuil(char* str)
{
   int i=0;
